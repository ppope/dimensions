...
* https://github.com/scikit-learn/scikit-learn/blob/114822b1e18c9d7f887c58b8a3b2c279bdce6d35/sklearn/utils/graph_shortest_path.pyx
"""
...
def graph_shortest_path(dist_matrix, directed=True, method='auto', first_n=None):
    """
    ...
    first_n : if given, only computes the nearest neighbors for the first n rows
    ...
    if first_n > N:
        raise ValueError("The value of first_n (%d) should not be larger than N (%d)"%(first_n, N))
    ...
    if method == 'FW':
        raise ValueError("FW is not well supported now")
    ...
    if method == 'FW':
        ...
        # print("modified3")
        graph = np.zeros((first_n, N), dtype=DTYPE, order='C')
        dijkstra(dist_matrix, graph, directed, first_n)
    else:
